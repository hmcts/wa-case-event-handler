...
public class MessageProcessorTest extends MessagingTests {

    @Test
    public void should_mark_case_event_messages_as_processed_in_database()  {
        ...
        messageIds.forEach(msgId ->
            saveMessageToDatabase(msgId, eventInformation)
        );
    ...
    private void saveMessageToDatabase(String msgId, EventInformation eventInformation) {
        EventInformationRequest eventInformationRequest = new EventInformationRequest(eventInformation, null);

        given()
            .contentType(APPLICATION_JSON_VALUE)
            .header(SERVICE_AUTHORIZATION, s2sToken)
            .body(asJsonString(eventInformationRequest))
            .when()
            .put("/messages/" + msgId + "?from_dlq=false");
    }
